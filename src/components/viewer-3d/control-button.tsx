'use client'

import type { ReactNode } from 'react'

interface ControlButtonProps {
  /** SVG path for the icon */
  icon: string | ReactNode
  /** Tooltip label */
  label: string
  /** Click handler */
  onClick?: () => void
  /** Whether button is in active state */
  isActive?: boolean
  /** Disable the button */
  disabled?: boolean
  /** Additional CSS classes */
  className?: string
  /** Size variant */
  size?: 'sm' | 'md'
}

/**
 * Reusable control button for viewer toolbar
 *
 * Features:
 * - Active state styling
 * - Hover tooltip
 * - Consistent sizing and transitions
 * - Support for both SVG path and ReactNode icons
 */
export function ControlButton({
  icon,
  label,
  onClick,
  isActive = false,
  disabled = false,
  className = '',
  size = 'md',
}: ControlButtonProps) {
  const sizeClasses = {
    sm: 'size-8',
    md: 'size-10',
  }

  const iconSizeClasses = {
    sm: 'w-4 h-4',
    md: 'w-5 h-5',
  }

  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`
        ${sizeClasses[size]}
        rounded-full flex items-center justify-center transition-all relative group
        ${isActive
          ? 'bg-primary/20 text-primary'
          : 'text-muted-foreground hover:text-foreground hover:bg-muted/10'
        }
        ${disabled ? 'opacity-50 cursor-not-allowed' : ''}
        ${className}
      `}
      aria-label={label}
    >
      {typeof icon === 'string' ? (
        <svg className={iconSizeClasses[size]} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={icon} />
        </svg>
      ) : (
        icon
      )}

      {/* Tooltip */}
      <span className="absolute bottom-full mb-2 hidden group-hover:block px-2 py-1 bg-popover text-popover-foreground text-xs rounded whitespace-nowrap border border-border pointer-events-none z-50">
        {label}
      </span>
    </button>
  )
}

// ============================================
// ICON PATHS (exported for reuse)
// ============================================

export const VIEWER_ICONS = {
  pan: 'M7 11.5V14m0-2.5v-6a1.5 1.5 0 113 0m-3 6a1.5 1.5 0 00-3 0v2a7.5 7.5 0 0015 0v-5a1.5 1.5 0 00-3 0m-6-3V11m0-5.5v-1a1.5 1.5 0 013 0v1m0 0V11m0-5.5a1.5 1.5 0 013 0v3m0 0V11',
  camera: 'M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z M15 13a3 3 0 11-6 0 3 3 0 016 0z',
  play: 'M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
  rotate: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15',
  layers: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10',
  fullscreen: 'M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4',
  menu: 'M4 6h16M4 12h16M4 18h16',
} as const
